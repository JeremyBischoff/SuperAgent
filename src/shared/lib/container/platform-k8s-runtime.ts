...
export class PlatformK8sRuntimeClient extends BaseContainerClient {
  ...
  static isEligible(): boolean {
    // TODO: use better container signal; this only means "running in a k8s pod".
  ...
  async start(options?: StartOptions): Promise<void> {
    ...
    if (!(await this.waitForHealthy(60_000, CONTAINER_INTERNAL_PORT))) {
      throw new Error(`Platform k8s agent pod ${this.podName()} failed to become healthy`)
    }
  }
...
function computeKubeConfigOrNull(): KubeConfig | null {
  ...
  const deploymentId = process.env.K8S_DEPLOYMENT_ID || process.env.SUPERAGENT_DEPLOYMENT_ID
  ...
  return {
    ...
    pvcName,
    orgSlug,
    orgId: process.env.SUPERAGENT_ORG_ID ?? null,
    runtimeVersion: process.env.SUPERAGENT_RUNTIME_VERSION ?? null,
...
async function assertNamespaceNotUpgrading(kube: KubeConfig): Promise<void> {
  try {
    ...
    console.warn('[PlatformK8sRuntimeClient] unable to read namespace upgrade annotation:', error)
    captureException(error, { tags: { area: 'container', op: 'k8s.namespaceUpgradeCheck' } })
  }
}
