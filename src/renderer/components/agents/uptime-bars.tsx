    ...
    <TooltipProvider delayDuration={150}>
      <div className="flex items-center gap-2 text-xs">
        <span className="min-w-0 flex-1 truncate">
          <span className="text-foreground">{label}</span> <span className="text-muted-foreground">Cron</span>
        </span>
        {/* Fixed column widths (68px graphic / 40px metric) keep the trailing
            graphic + metric aligned with the webhook-health row in the cards. */}
        <div className="flex items-center gap-0.5 shrink-0 w-[68px]">
          {runs.map((run, i) => {
