...
export function setupBrowserStreamProxy(server: ServerType): void {
  ...
  browserWss.on('connection', async (ws: WebSocket, request: IncomingMessage) => {
    ...
    try {
        ...
        // Connect to the container's browser stream WebSocket through the
        // runtime's WS origin so cloud (Kubernetes) goes via cluster DNS,
        // not the host-app pod's localhost.
